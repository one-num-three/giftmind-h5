...
import { openInTaobao, copyTaobaoToken } from '@/utils/taobaoBridge'
...
function onOpenTaobao(e) {
  e?.stopPropagation?.()
  const giftName = name.value
  const brand = g.value.brand || ''
  openInTaobao(giftName, brand)
}

async function onCopyTaobao(e) {
  e?.stopPropagation?.()
  const giftName = name.value
  const price = g.value.price || explanation.value?.price || ''
  const ok = await copyTaobaoToken(giftName, price)
  if (ok) {
    ui.success('淘宝搜索口令已复制！打开【手机淘宝】即可直接查看')
  } else {
    ui.error('复制失败，请手动打开淘宝搜索')
  }
}
</script>
...
<template>
  ...
  >
    ...
    <div class="gift__inner">
      ...
      <!-- ── 淘宝直达与口令工具栏 ── -->
      <div class="gift__taobao-toolbar">
        <button
          type="button"
          class="tb-btn tb-btn--primary tap"
          title="手机端直接唤起手机淘宝App，电脑端打开天猫/淘宝正品搜索"
          @click="onOpenTaobao"
        >
        ...
        <button
          type="button"
          class="tb-btn tb-btn--secondary tap"
          title="复制淘口令搜索文本"
          @click="onCopyTaobao"
        >
...
.tb-btn--primary {
  background: linear-gradient(135deg, #ff7a45 0%, #ff4d12 100%);
  color: #ffffff;
  box-shadow: 0 3px 10px rgba(255, 77, 18, 0.25);
}

.tb-btn--primary:hover {
  background: linear-gradient(135deg, #ff8c5a 0%, #ff5d24 100%);
}

.tb-btn--primary:active {
  transform: scale(0.97);
  box-shadow: 0 1px 4px rgba(255, 77, 18, 0.2);
}

.tb-btn--secondary {
  background: var(--c-surface-sunken, #f7f4ee);
  color: var(--c-ink-2, #555);
  border: 1px solid var(--c-line-soft, #e8e3d8);
}

.tb-btn--secondary:hover {
  background: var(--c-surface-soft, #ede8dc);
}
