...
export const useSessionStore = defineStore('session', {
  ...
  getters: {
    ...
    isFinished() {
      return this.isAiReady || this.stepIndex >= 15
    },
    progress() {
      return progressOf(this.stepIndex, Math.max(6, this.stepIndex + 1))
    },
    stageText() {
  ...
  actions: {
    ...
    addExistingItem(item) {
      if (item && !this.existingItems.includes(item)) {
        this.existingItems.push(item)
      }
    },

    backtrackCategory(reason = '') {
      // 捕获回退原因作为负向约束
      if (reason) {
        this.negativeConstraints[reason] = true
      }
      delete this.answers.concrete_object
      delete this.answers.category_choice
      delete this.answers.lego_theme
      delete this.answers.pillow_scenario
      delete this.answers.pillow_firmness
      delete this.answers.pillow_material
      delete this.answers.tea_type
      delete this.answers.lamp_type
      delete this.answers.figure_type
      delete this.answers.jewelry_type
      delete this.answers.digital_type
      this.categoryAttributes = {}
      this.stepIndex = Math.max(1, this.stepIndex - 1)
      this.persistDraft()
    },
